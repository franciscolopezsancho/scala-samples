...
object InAndOut extends App {
  ...
  class NetworkSwitch(val name: String) { thisSwitch =>
    ...
    // as port1 don't belongs to networkSwitchB
  ...
  // This is quite useful as it's quite dangerous to lose the reference to the 
  // parent. In a piece of code that doesn't keep the context this way once
  // the parent is not in scope this is the only way to recover it.
